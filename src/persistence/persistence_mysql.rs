...
impl Db for MysqlDb<'_> {
    ...
    fn task_create(&self, params: &TaskCreateParams) -> StorageResult<Option<TaskCreateResult>> {
        ...
        Ok(Some(TaskCreateResult {
            promise,
            task_created,
            task_state: if task_created { Some(task_initial_state.to_string()) } else { None },
        }))
    }
    ...
    fn task_suspend(
        ...
        if settled_count == 0 {
            // 6. Can suspend: insert callbacks, remove task timeout, transition task
    ...
    fn task_release(
        ...
        let released = res.rows_affected() > 0;
    ...
    fn task_continue(&self, task_id: &str, time: i64) -> StorageResult<TaskContinueResult> {
        ...
        let state_row = rt_block_on(
            sqlx::query("SELECT state FROM tasks WHERE id = ?")
                .bind(task_id).fetch_optional(self.tx().as_mut())
        )?;

        Ok(TaskContinueResult {
            state: state_row.map(|r| parse_task_state(&r.get::<String, _>("state"))),
            continued,
        })
    }
    ...
    fn process_schedule_timeout(
        ...
        // created_at mirrors model semantics: when the tick time already exceeds the
        // promise's timeout, use timeoutAt (the promise was logically never pending).
        // Otherwise use the tick time itself.
        let created_at = if time >= computed_timeout_at { computed_timeout_at } else { time };

        // 5. Create promise (idempotent)
        let promise_res = rt_block_on(
            sqlx::query(
                "INSERT IGNORE INTO promises
                 (id, state, param_headers, param_data, tags, timeout_at, created_at)
                 VALUES (?, 'pending', ?, ?, ?, ?, ?)"
