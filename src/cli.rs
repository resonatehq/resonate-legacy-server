...
pub struct CommonArgs {
    ...
    #[arg(long = "storage-mysql-url", env = "RESONATE_STORAGE__MYSQL__URL")]
    pub mysql_url: Option<String>,

    /// MySQL connection pool size [default: 10]
    #[arg(long = "storage-mysql-pool-size", env = "RESONATE_STORAGE__MYSQL__POOL_SIZE")]
...
impl CommonArgs {
    fn apply(self, mut config: crate::config::Config) -> crate::config::Config {
        ...
        if let Some(pool_size) = self.mysql_pool_size {
            config.storage.mysql.pool_size = pool_size;
        }
